...
class ConfluenceClient:
    ...
    def move_page(
        self, page_id: str, target_space_key: str, title: str, body: str,
        version: int, parent_id: str | None = None,
    ) -> dict:
        """Move a page to a different space and update its content.

        Uses v1 API for cross-space moves (v2 PUT does not reliably support
        changing spaceId). This preserves owner, version history, comments,
        and attachments.
        """
        payload = {
            "type": "page",
        ...
        return self._put(f"/wiki/rest/api/content/{page_id}", payload)
